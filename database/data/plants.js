export default [
  {
    name: 'Broadleaf Lady Palm',
    sciName: 'Rhapis Excelsa',
    image: ' ',
    origin: 'China',
    wateringLevel: 'Every 7-10 days',
    wateringCategory: 'High',
    tempRange: '-6-37°C',
    tempCategory: 'Very Low to Very High',
    lightType: 'Indirect',
    brightType: 'Partially bright',
    problems: [
      {
        brownLeaves: 'Can be caused by extreme heat or lack of water. Trim the leaves and address the issues.',
        yellowLeaves: ' ',
        blackLeaves: 'Can be caused by over-watering or over-fertilizing. Trim the leaves and address the issues.',
        baseRot: ' ',
        leafWilting: ' ',
        bugs: ' ',
        leafLoss: 'This can happen with age. Make sure you prune to keep the plant healthy',
      }
    ],
    otherProblems: [
      {
        problem: ' ',
        solution: ' ',
      }
    ],
    maxGrowth: '4 metres',
    annualGrowth: '20-30cm',
    poisonous: 'Non toxic for cats and dogs.',
    easeRating: 4,
    description: 'The Broadleaf Lady Palm is a multi-stemmed palm plant that is becoming very popular around the world because of it\'s air cleaning abilities. Unlike other plants that only make oyxgen, it can remove dangerous chemicals in the air like carbon dioxide and ammonia. Broadleaf Lady Palms are popular both as indoor and outdoor plants but may need some extra pruning to be kept in check if kept indoors.',
    priceRange: '£30-120',
    buyLink: ' ',
    seededSays: 'Broadleaf Lady Palms are great in most environments. Make sure to water them every 7 - 10 days to keep their soil constantly moist but never soggy! They enjoy indirect sunlight and are fine with almost all temperatures so you can place them anywhere. Just make sure to give them a bit of shade to keep them happy. They can get a little overgrown so you can prune them when they start to get too large to handle.',
  },
  {
    name: 'Cheese Plant',
    sciName: 'Monstera Deliciosa',
    image: ' ',
    origin: 'Central America',
    wateringLevel: 'Every 7-14 days',
    wateringCategory: 'Medium',
    tempRange: '18-27°C',
    tempCategory: 'Average to High',
    lightType: 'Indirect',
    brightType: 'Mixed',
    problems: [
      {
        brownLeaves: 'Can be caused by low humidity and dry air. Changing the pot or adjusting the humidity of the room could work.',
        yellowLeaves: 'Can be caused through over-watering. Stop this. If that doesn\'t help it might need fertilizer.',
        blackLeaves: ' ',
        baseRot: ' ',
        leafWilting: ' ',
        bugs: ' ',
        leafLoss: ' ',
      }
    ],
    otherProblems: [
      {
        problem: 'Leaves not forming slits or holes',
        solution: 'Could be the lack of: light, water or fertilizer',
      }
    ],
    maxGrowth: '3 metres',
    annualGrowth: '30-60cm',
    poisonous: 'Non toxic for cats and dogs.',
    easeRating: 5,
    description: 'The Swiss cheese plant is a type of flowering plant originally from the tropical forests of southern Mexico. Some of types of cheese plants can even grow fruit that tastes like a mix of strawberries, passion fruit, mango, and pineapple.',
    priceRange: '£10-100',
    buyLink: ' ',
    seededSays: 'Cheese plants are very easygoing and only need to be watered every 1 - 2 weeks. Letting the top soil dry out between watering times is perfectly fine. Cheese plants love lots of indirect sunlight so make sure to keep them in bright rooms with plent of shade. The temperature of the room won\'t matter too much but their growth can be slowed down if it gets too cold. They can easily get out of hand once it\'s been growing for a while and will need cutting back.',
  },
  {
    name: 'Snake Plant',
    sciName: 'Dracaena trifasciata',
    image: ' ',
    origin: 'West Africa',
    wateringLevel: 'Every 14-28 days',
    wateringCategory: 'Low',
    tempRange: '15-24°C',
    tempCategory: 'Average',
    lightType: 'Mixed',
    brightType: 'Mixed',
    problems: [
      {
        brownLeaves: ' ',
        yellowLeaves: ' ',
        blackLeaves: ' ',
        baseRot: 'Can be caused by over-watering in cold conditions. You will need to remove the affected parts or discard the plant if the whole base if affected.',
        leafWilting: ' ',
        bugs: ' ',
        leafLoss: ' ',
      }
    ],
    otherProblems: [
      {
        problem: ' ',
        solution: ' ',
      }
    ],
    maxGrowth: '1.5 metres',
    annualGrowth: '5-15cm',
    poisonous: 'Mildly toxic for cats and dogs.',
    easeRating: 4,
    description: 'Snake plants are on the top of the list of air purifying plants. The unusual thing they do is that they actually release oxygen at night instead of during the day. They are originally from  West Africa and have many common names. They are commonly known as the snake plant, Saint George\'s sword, mother-in-law\'s tongue, and viper\'s bowstring hemp.',
    priceRange: '£10-100',
    buyLink: ' ',
    seededSays: 'Snake plants store water in their foliage so they only need to be watered once or twice a month. From Spring to Autumn water when the soil becomes dry but only once a month during winter. They\'re happy in almost any environment but will grow faster when they have lots of indirect sunlight. Too much direct sunlight could give them a nasty sunburn. They don\'t need fertiliser but you could give them succulent fertiliser once a month to help it grow.',
  },
  {
    name: 'Money Tree Plant',
    sciName: 'Pachira Aquatica',
    image: ' ',
    origin: 'South & Central America',
    wateringLevel: 'Every 7-14 days',
    wateringCategory: 'Medium',
    tempRange: '18-24°C',
    tempCategory: 'Average',
    lightType: 'Indirect',
    brightType: 'Bright',
    problems: [
      {
        brownLeaves: 'Could be cause by dry air, too much or too little light or water, or maybe cold drafts. This one\'s a process of elimination',
        yellowLeaves: 'Can be caused by over-feeding or over-watering. Both activities should be stopped and watering resumed when the soil is dry',
        blackLeaves: ' ',
        baseRot: ' ',
        leafWilting: 'Can be caused by sudden changes in temperature and lighting conditions. This might mean your plant needs to move.',
        bugs: ' ',
        leafLoss: ' ',
      }
    ],
    otherProblems: [
      {
        problem: ' ',
        solution: ' ',
      }
    ],
    maxGrowth: '3 metres',
    annualGrowth: '20-50cm',
    poisonous: 'Non toxic for cats and dogs.',
    easeRating: 4,
    description: 'The Pachira aquatica has many common names including Mexican fortune tree, braided money tree, malabar chestnut, guiana chestnut and a few others. Growing in it\'s natural habitat - this native to south and central America plant grows in swamps up to 20 metres tall, which is surprising considering how well it\'s adapted to become an easy to care for indoor tree.',
    priceRange: '£10-150',
    buyLink: ' ',
    seededSays: ' ',
  },
  {
    name: 'Madagascar Dragon Tree',
    sciName: 'Dracaena Marginata',
    image: ' ',
    origin: 'Madagascar',
    wateringLevel: 'Every 7 days',
    wateringCategory: 'High',
    tempRange: '18-24°C',
    tempCategory: 'Average',
    lightType: 'Indirect',
    brightType: 'Bright',
    problems: [
      {
        brownLeaves: 'Could be cause by dry air, cold drafts or under-watering. You should assess the conditions and identify the issue.',
        yellowLeaves: 'If at the bottom, this is normal. The plant is shdding leaves to allow new growth.',
        blackLeaves: ' ',
        baseRot: ' ',
        leafWilting: ' ',
        bugs: ' ',
        leafLoss: ' ',
      }
    ],
    otherProblems: [
      {
        problem: ' ',
        solution: ' ',
      }
    ],
    maxGrowth: '3 metres',
    annualGrowth: '10-30cm',
    poisonous: 'Mildly toxic for cats and dogs.',
    easeRating: 4,
    description: 'The Madagascar Dragon Tree is most definitely one of the easiest indoor plants to grow and maintain. Dracaena Marginata trees can grow up to 6ft high indoors, and they\'re slow growing.',
    priceRange: '£10-60',
    buyLink: ' ',
    seededSays: ' ',
  }
] 